...
def main(cfg, args):
    ...
    models = {
        "ThermoMPNN": get_trained_model(
            model_name=args.model_path, config=cfg, override_custom=True
        )
    }
    ...
    assert os.path.isdir(args.out_dir), print(
        f"{args.out_dir} is not a valid directory."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pdb", type=str, default="", help="Input PDB to use for custom inference"
    )
    parser.add_argument(
        "--chain", type=str, default="A", help="Chain in input PDB to use."
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default="",
        help="filepath to model to use for inference",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="./",
        help="Output directory in which to save predictions.",
    )
