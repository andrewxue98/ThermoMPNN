...
def main(cfg, args):
    ...
    models = {
        ...
        "ThermoMPNN": get_trained_model(
            model_name="example_training_mega_epoch=55_val_ddG_spearman=0.79.ckpt",
            config=cfg,
        ),
        ...
        for dataset_name, dataset in datasets.items():
            ...
            for i, batch in enumerate(tqdm(dataset)):
                ...
                if args.pick_best:
                    ...
                    current_slice = raw_pred_df.loc[
                        raw_pred_df["pdb"] == mut.pdb.strip(".pdb")
                    ]
                    best_res_list = retrieve_best_mutants(
                        current_slice, allow_cys=args.include_cys
                    )
                    for p, b in zip(current_slice.position.unique(), best_res_list):
                        raw_pred_df.loc[
                            (raw_pred_df["pdb"] == mut.pdb.strip(".pdb"))
                            & (raw_pred_df["position"] == p),
                            "best_AA",
                        ] = b

                    # only keep one row per position for AA pattern detection
                    raw_pred_df["dupe_detector"] = raw_pred_df["pdb"] + raw_pred_df[
                        "position"
                    ].astype(str)
                    raw_pred_df = raw_pred_df.drop_duplicates(
                        subset=["dupe_detector"], keep="first"
                    )
