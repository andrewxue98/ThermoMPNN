...
class TransferModelPL(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if (
            self.lr_schedule
        ):  # enable additional lr scheduler conditioned on val ddG mse
...
def train(cfg):
    ...
    if "num_workers" in cfg.training:
        train_workers, val_workers = int(cfg.training.num_workers * 0.75), int(
            cfg.training.num_workers * 0.25
        )
    else:
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor=monitor, mode="max", dirpath="checkpoints", filename=filename
    )
    logger = (
        WandbLogger(project=cfg.project, name="test", log_model="all")
        if "project" in cfg
        else None
    )
